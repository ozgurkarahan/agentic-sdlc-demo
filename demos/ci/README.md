# `demos/ci/` — real verification logic (Tier-1 runnable, Tier-2 enforceable)
...
(`demos/validate/`) can call it directly **with no GitHub**, and the same logic becomes
...
> The dispatcher's plan-approval gate (`demos/orchestrator/`) is **separate** layered
...
node demos/ci/scripts/eval-rubric.mjs --app demos/sample-app/dist/app.js \
  --variant demos/fixtures/quality-test/good.mjs --max 3        # PASS (exit 0)
node demos/ci/scripts/eval-rubric.mjs --app demos/sample-app/dist/app.js \
  --variant demos/fixtures/quality-test/no-429.mjs --max 3      # FAIL (exit 1)
...
node demos/ci/scripts/trajectory-check.mjs \
  --declared demos/sample-app/src/middleware/ \
  --changed demos/sample-app/src/middleware/rateLimit.ts \
  --required-test demos/sample-app/test/unit/middleware/rateLimit.test.ts   # FAIL (no test)
node demos/ci/scripts/path-scope-check.mjs \
  --declared demos/sample-app/src/middleware/ \
  --changed demos/sample-app/src/middleware/rateLimit.ts,demos/sample-app/src/config.ts  # FAIL (stray)
...
```bash
node demos/validate/run.mjs           # human-readable matrix, exit 0 only if all correct
node demos/validate/run.mjs --json    # machine-readable
node demos/validate/run.mjs --filter security-compliance   # one agent
```
`demos/validate/run.mjs` loads every `demos/fixtures/<agent>/*.json` (CONTRACT §4 schema),
...
a stronger guarantee than it is. See `demos/validate/README.md`.
...
```
demos/sample-app/*          ->  <repo root>            (app, tests, package.json, lockfile)
demos/ci/scripts/*          ->  <repo root>/ci/scripts/
demos/ci/workflows/*.yml    ->  <repo root>/.github/workflows/
